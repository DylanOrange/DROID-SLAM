...
class UpdateModule(nn.Module):
    ...
    def forward(self, net, inp, corr, flow=None, ii=None, jj=None):
        ...
        flow_dy = self.flow_encoder(flow[:,4:])#4->64
        flow = self.flow_integrater(torch.cat([flow_st, flow_dy], dim=1))
        ...
        dydelta = self.delta_dy(delta).view(*output_dim)
        ...
        dydelta = dydelta.permute(0,1,3,4,2)[...,:2].contiguous()
        ...
        # else:
        return net, delta, weight, dydelta, dyweight
...
class DroidNet(nn.Module):
    ...
    def forward(self, Gs, Ps, ObjectGs, ObjectPs, images, objectmasks, highmasks, \
        ...
        gtflow_st, _ = pops.projective_transform(Ps, disps, intrinsics, ii, jj)
        ...
        all_Gs_list, all_disp_list, all_ObGs_list, all_flow_list, all_dyflow_list, all_static_residual_list,  all_dyna_residual_list= [], [], [], [], [], [], []
        ...
        for index in range(1):
            ...
            Gs_list, disp_list, ObGs_list, flow_list, dyflow_list, static_residual_list, dyna_residual_list = [], [], [], [], [], [], []
            ...
            for step in range(num_steps):
                ...
                target_dyna = target_dyna.detach()

                # extract motion features
                corr = corr_fn(coords1)
                resd = target - coords1
                flow = coords1 - coords0
                dyflow = coordsdyna - coords1
                dyresd = target_dyna - coordsdyna

                motion = torch.cat([flow, resd, dyflow, dyresd], dim=-1)
                motion = motion.permute(0,1,4,2,3).clamp(-64.0, 64.0)

                net, delta, weight, dydelta, dyweight = \
                    self.update(net, inp, corr, motion, ii, jj)

                target = coords1 + delta
                target_dyna = coordsdyna + dydelta
                ...
                print('predicted dynamic flow loss is {}'.format((gtflow - target_dyna)[objectmasks[:,ii]>0.5].mean().item()))
                ...
                for i in range(2):
                    ObjectGs = dynamicBA(target_dyna, dyweight, ObjectGs, objectmasks, trackinfo, validmask, \
                ...
                coordsdyna, valid_dyna = pops.dyprojective_transform(Gs, disps, intrinsics, ii, jj, \
                                                                    validmask, ObjectGs, objectmasks)
                static_residual = (target - coords1)*valid_static
                dyna_residual = (target_dyna - coordsdyna)*valid_dyna
