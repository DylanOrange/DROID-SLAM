...
class DepthVideo:
    ...
    def ba(self, target, weight, eta, ii, jj, t0=1, t1=None, itrs=2, lm=1e-4, ep=0.1, motion_only=False):
        ...
        with self.get_lock():
            ...
            # print('before op {}'.format(self.disps[t0:t1].mean()))
            droid_backends.ba(self.poses, self.disps, self.intrinsics[0], self.disps_sens,
                target, weight, eta, ii, jj, t0, t1, itrs, lm, ep, True)
            # print('after op {}'.format(self.disps[t0:t1].mean()))
