...
def dynamicBA(target, weight, objectposes, objectmask, app, validmask, eta, poses, disps, intrinsics, ii, jj, fixedp=0):
    ...
    useless = torch.nonzero((r-r.min() > 0.7*(r.max()-r.min())), as_tuple = True)

    # histc = torch.histc(r,100)
    # bins = 100
    # x = range(bins)
    # plt.bar(x, histc.cpu().numpy(), align='center')
    # plt.xlabel('Bins')

    r = (target - coords).view(B, N, -1, 1) #1,18,30,101,2-> 1,18,6060,1
    # w = .001*(valid*weight).view(B,N,-1,1) #1,18,3030,1
    w = (valid*weight).repeat(1,1,1,1,2)
    w[useless]=0
    w = w.view(B,N,-1,1)
    ...
    ho = scatter_sum(Joi, ii_scatter, dim = 1, dim_size= N*P) + scatter_sum(Joj, jj_scatter, dim = 1, dim_size= N*P)
    ho = ho.view(B, N, P, -1, D).permute(0,1,3,2,4)#1,18,6,6060,3
    ho = ho[:, :, :, fixedp:].reshape(B, N, -1, (P-fixedp)*D)
    ...
    # dx = block_solve_initial(H, v[:,5:])
    # print('update value is {}'.format(dx.mean().item()))
    P = P-fixedp
    dx = dx.reshape(-1, P, D)
    # poses = pose_retr(poses, dx[:,:P], torch.arange(P).to(device=dx.device) + fixedp)
    objectposes = pose_retr(objectposes, dx, torch.arange(P).to(device=dx.device) + fixedp)
