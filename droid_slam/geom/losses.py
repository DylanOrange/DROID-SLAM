...
def residual_loss(residuals, dyna = False, gamma=0.9):
    ...
    for index in range(len(residuals)):
        ...
        if dyna:
            metrics = {'dyna residual': residual_loss.item()}
        else:
...
def flow_loss(Ps, disps, highdisps, poses_est, disps_est, ObjectPs, objectposes_est, \
              objectmasks, highobjectmask, trackinfo, intrinsics, graph, flow_list, dyflow_list, scale, gamma=0.9):
    """ optical flow loss """
    ...
    lowgtflow_st, lowmask_st = projective_transform(Ps, disps, intrinsics, ii, jj)
    ...
    lowmask_st = lowmask_st * (disps[:,ii] > 0).float().unsqueeze(dim=-1)
    ...
    for i in range(n):
        ...
        #low resolution flow
        i_error_low = lowmask*(lowgtflow_st - flow_list[0][i]).abs()
        # print('low flow error is {}'.format(i_error_low.mean().item()))
        error_lowflow += w*(i_error_low.mean())

        #low resolution dyna flow
        i_error_dylow = lowmask*(lowgtflow - dyflow_list[0][i]).abs()
        i_error_dylow = i_error_dylow[objectmasks[:,ii]>0.5]
        error_dylow += w * i_error_dylow.mean()

        #low resolution pose and depth

        flow_low_induced_st, lowmask_induced_st = projective_transform(poses_est[0][i], disps_est[0][i], \
                                                                   intrinsics, ii, jj)
        ...
        low_v = (lowmask_induced_st * lowmask_st).squeeze(dim=-1)
        i_error_induced_st = low_v * (lowgtflow_st - flow_low_induced_st).norm(dim=-1)
        error_induced_st += w * i_error_induced_st.mean()

        low_v = (lowmask_induced * lowmask).squeeze(dim=-1)
        i_error_induced_dyna = low_v * ((lowgtflow - lowgtflow_st) - (flow_low_induced - flow_low_induced_st)).norm(dim=-1)
        error_induced_dyna += w * i_error_induced_dyna[objectmasks[:,ii]>0.5].mean()
    ...
    epe_induced_st = i_error_induced_st.reshape(-1)[low_v.reshape(-1)>0.5]
    epe_induced_dyna = i_error_induced_dyna[objectmasks[:,ii]>0.5]
    ...
    metrics = {
        ...
        'low_f_induced_error': epe_induced_st.mean().item(),
        'low_induced_1px': (epe_induced_st<1.0).float().mean().item(),
    ...
    return error_lowflow, error_dylow, error_induced_st, error_induced_dyna, error_lowdepth, error_lowdynadepth, \
            metrics