...
class RGBDAugmentor:
    ...
    def spatial_transform(self, images, objectmasks, depths, poses, intrinsics):
        ...
        objectmasks = objectmasks.unsqueeze(dim=1)
        ...
        objectmasks = F.interpolate(objectmasks, scale_factor=scale, recompute_scale_factor=True)
        ...
        objectmasks = objectmasks[:, :, y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]

        depths = depths.squeeze(dim=1)
        objectmasks = objectmasks.squeeze(dim=1).int()
        return images, objectmasks, poses, depths, intrinsics
