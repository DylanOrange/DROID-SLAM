...
class RGBDAugmentor:
    ...
    def spatial_transform(self, images, depths, midasdisps, poses, intrinsics):
        ...
        midasdisps = midasdisps.unsqueeze(dim=1)
        ...
        midasdisps = F.interpolate(midasdisps, scale_factor=scale, recompute_scale_factor=True)
        ...
        midasdisps = midasdisps[:, :, y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]

        depths = depths.squeeze(dim=1)
        midasdisps = midasdisps.squeeze(dim=1)
        return images, poses, depths, midasdisps, intrinsics
    ...
    def __call__(self, images, poses, depths, midasdisps, intrinsics):
        images = self.color_transform(images)
        return self.spatial_transform(images, depths, midasdisps, poses, intrinsics)
